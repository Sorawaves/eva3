...
if [[ $GITHUB_REPO == *"TU_USUARIO"* ]]; then
    echo "   ⚠️  El proyecto debe estar en /home/$WEB_USER/Evaluacion3, /tmp/Evaluacion3, /tmp/logistica o /home/$WEB_USER/logistica"
    
    if [ -d "/home/$WEB_USER/Evaluacion3" ]; then
        echo "   ✅ Copiando desde /home/$WEB_USER/Evaluacion3..."
        cp -r "/home/$WEB_USER/Evaluacion3" "$PROJECT_DIR"
    elif [ -d "/tmp/Evaluacion3" ]; then
        echo "   ✅ Copiando desde /tmp/Evaluacion3..."
        ...
        echo "   Sube el proyecto primero con scp desde tu máquina local:"
        echo "   scp -i tu-llave.pem -r /ruta/local/proyecto $WEB_USER@${IP_PUBLICA}:/tmp/logistica"
        exit 1
    fi
else
