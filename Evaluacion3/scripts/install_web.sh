...
# Detectar usuario del sistema según el OS
if [ "$OS" = "amzn" ]; then
    WEB_USER="ec2-user"
    WEB_GROUP="nginx"
else
...
echo "👤 Usuario del sistema: $WEB_USER"
...
if [[ $GITHUB_REPO == *"TU_USUARIO"* ]]; then
    echo "   ⚠️  Buscando proyecto en ubicaciones conocidas..."
    
    if [ -d "/home/$WEB_USER/eva3/Evaluacion3" ]; then
        echo "   ✅ Encontrado en /home/$WEB_USER/eva3/Evaluacion3"
        mkdir -p "$PROJECT_DIR"
        cp -r /home/$WEB_USER/eva3/Evaluacion3/* "$PROJECT_DIR/"
    elif [ -d "/home/$WEB_USER/Evaluacion3" ]; then
        echo "   ✅ Encontrado en /home/$WEB_USER/Evaluacion3"
        mkdir -p "$PROJECT_DIR"
        cp -r /home/$WEB_USER/Evaluacion3/* "$PROJECT_DIR/"
    elif [ -d "/tmp/Evaluacion3" ]; then
        echo "   ✅ Encontrado en /tmp/Evaluacion3"
        mkdir -p "$PROJECT_DIR"
        cp -r /tmp/Evaluacion3/* "$PROJECT_DIR/"
    elif [ -d "/home/$WEB_USER/logistica" ]; then
        echo "   ✅ Encontrado en /home/$WEB_USER/logistica"
        mkdir -p "$PROJECT_DIR"
        cp -r /home/$WEB_USER/logistica/* "$PROJECT_DIR/"
    elif [ -d "/tmp/logistica" ]; then
        echo "   ✅ Encontrado en /tmp/logistica"
        mkdir -p "$PROJECT_DIR"
        cp -r /tmp/logistica/* "$PROJECT_DIR/"
    else
        echo "   ❌ No se encontró el proyecto en ninguna ubicación"
        echo "   Ubicaciones buscadas:"
        echo "     - /home/$WEB_USER/eva3/Evaluacion3"
        echo "     - /home/$WEB_USER/Evaluacion3"
        echo "     - /tmp/Evaluacion3"
        echo "     - /home/$WEB_USER/logistica"
        echo "     - /tmp/logistica"
        echo ""
        echo "   Sube el proyecto con: scp -i tu-llave.pem -r /ruta/proyecto $WEB_USER@${IP_PUBLICA}:/tmp/Evaluacion3"
        exit 1
    fi
else
