...
if [ "$OS" = "amzn" ] || [ "$OS" = "rhel" ] || [ "$OS" = "centos" ]; then
    ...
    yum install -y https://download.postgresql.org/pub/repos/yum/reporpms/EL-7-x86_64/pgdg-redhat-repo-latest.noarch.rpm 2>/dev/null || true
    ...
    /usr/bin/postgresql-14-setup initdb 2>/dev/null || /usr/pgsql-14/bin/postgresql-14-setup initdb
